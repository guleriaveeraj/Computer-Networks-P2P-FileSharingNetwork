...
public class MessageProcessor implements Runnable, Constants 
{
	...
	public void run()
	{
		...
		while(running)
		{
			...
			if(msgType.equals(M_HAVE ) && state != 14)
			{
			...
			else
			{
			 ...
			 case 3:
			   if (msgType.equals(M_NOT_INTERESTED )) {
				 // LOG 9:
			   ...
			   else if (msgType.equals(M_INTERESTED )) {
				// LOG 8:
			 ...
			 case 11:
				 if (msgType.equals(M_PIECE )) {
						...
						peerProcess.remotePeerInfoHash.get(rPeerId).dataRate = ((double)(buffer.length + LEN_M_LENGTH  + LEN_M_TYPE )/(double)timeLapse) * 100;
				 ...
				 else if (msgType.equals(M_CHOKE )) {
					// LOG 6:
						peerProcess.showLog(peerProcess.peerID + " is CHOKED by Peer " + rPeerId);
						peerProcess.remotePeerInfoHash.get(rPeerId).state = 14;
				 }
	...
	private void sendRequest(Socket socket, int pieceNo, String remotePeerID) {
		...
		// Byte2int....
		byte[] pieceByte = new byte[Constants.LEN_PIECE_INDEX ];
		for (int i = 0; i < Constants.LEN_PIECE_INDEX ; i++) {
			pieceByte[i] = 0;
		}
		...
		Message d = new Message(M_REQUEST , pieceByte);
		byte[] b = Message.encodeMessage(d);
		SendData(socket, b);
	...
	private void sendPeice(Socket socket, Message d, String remotePeerID)  //d == requestmessage
	{
		...
		byte[] buffer = new byte[noBytesRead + Constants.LEN_PIECE_INDEX ];
		System.arraycopy(bytePieceIndex, 0, buffer, 0, Constants.LEN_PIECE_INDEX );
		System.arraycopy(byteRead, 0, buffer, Constants.LEN_PIECE_INDEX , noBytesRead);

		Message sendMessage = new Message(M_PIECE , buffer);
		byte[] b =  Message.encodeMessage(sendMessage);
		SendData(socket, b);
	...
	private void sendNotInterested(Socket socket, String remotePeerID) 
	{
		...
		Message d =  new Message(M_NOT_INTERESTED );
		byte[] msgByte = Message.encodeMessage(d);
	...
	private void sendInterested(Socket socket, String remotePeerID) {
		...
		Message d =  new Message(M_INTERESTED );
		byte[] msgByte = Message.encodeMessage(d);
		SendData(socket,msgByte);		
	}
...
//  Compare the bitfield and send TRUE if there is any extra data	
	private boolean isInterested(Message d, String rPeerId) {		
	...
	private void sendUnChoke(Socket socket, String remotePeerID) {
		...
		Message d = new Message(M_UNCHOKE );
		byte[] msgByte = Message.encodeMessage(d);
		SendData(socket,msgByte);
	}
	...
	private void sendBitField(Socket socket, String remotePeerID) {
		...
		Message d = new Message(M_BITFIELD , encodedBitField);
		SendData(socket,Message.encodeMessage(d));
	...
	private void sendHave(Socket socket, String remotePeerID) {
		...
		byte[] encodedBitField = peerProcess.ownBitField.encode();
		Message d = new Message(M_HAVE , encodedBitField);
		SendData(socket,Message.encodeMessage(d));
