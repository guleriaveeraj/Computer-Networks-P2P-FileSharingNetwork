...
{private HandshakeMessage handshakeMessage;
	...
	public boolean receivePeice()
	{
		...
		{peerProcess.showLog(this.ownPeerId + " receivePeice : " + e.getMessage());return false;}
	...
	{	
	   ...
	   this.peerSocket = new Socket(add, port);			
		...
		try 
		{in = peerSocket.getInputStream();out = peerSocket.getOutputStream();} 
		catch (Exception ex) 
...
public boolean SendNotInterested()
	...
	{
		try{out.write(DataMessage.encodeMessage(new DataMessage(DATA_MSG_INTERESTED)));} 
		catch (IOException e){peerProcess.showLog(this.ownPeerId + " SendInterested : " + e.getMessage());return false;}
		return true;
	}
	...
	public boolean ReceiveChoke()
	{
		...
		try{if(in.available() == 0) return false;} 
		catch (IOException e){peerProcess.showLog(this.ownPeerId + " ReceiveChoke : " + e.getMessage());return false;
		try{in.read(receiveChokeByte);} 
		catch (IOException e){
			peerProcess.showLog(this.ownPeerId + " ReceiveChoke : " + e.getMessage());return false;}
		DataMessage m = DataMessage.decodeMessage(receiveChokeByte);
		if(m.getMessageTypeString().equals(DATA_MSG_CHOKE))
		{
			// LOG 6:
			peerProcess.showLog(ownPeerId + " is CHOKED by " + remotePeerId);return true;}
		else 
			return false;
	}
	...
	{	byte []handshakeBuff = new byte[32];
		...
		try{if(this.connType == ACTIVECONN)
		{if(!SendHandshake())
		{peerProcess.showLog(ownPeerId + "Your HANDSHAKE sending has been failed.");
		System.exit(0);}
		else{	peerProcess.showLog(ownPeerId + " HANDSHAKE has been sent...");}
		while(true) {
		...
		//populate peerID to socket mapping
			peerProcess.peerIDToSocketMap.put(remotePeerId, this.peerSocket);break;}
		else{continue;}		}
				// Sending BitField...
	...
	{while(true){
		in.read(handshakeBuff);
			...
			{	continue;}}
			if(!SendHandshake())
				{peerProcess.showLog(ownPeerId + " HANDSHAKE message sending failed.");System.exit(0);}
				else
	...
	if(dataMessage.getMessageTypeString().equals(MessageConstants.DATA_MSG_CHOKE)||dataMessage.getMessageTypeString().equals(MessageConstants.DATA_MSG_UNCHOKE)||dataMessage.getMessageTypeString().equals(MessageConstants.DATA_MSG_INTERESTED)
	...
	bytesAlreadyRead = 0;
	bytesRead = 0;}	}}
		catch(IOException e){	peerProcess.showLog(ownPeerId + " run exception: " + e);}	}
	
	public void releaseSocket() {
		try {if (this.connType == PASSIVECONN && this.peerSocket != null) {this.peerSocket.close();}
			if (in != null) {	in.close();}
			if (out != null)
				out.close();} 
			catch (IOException e) {peerProcess.showLog(ownPeerId + " Release socket IO exception: " + e);}}
}